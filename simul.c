...
{
    tceu_sim_path* path = &S.state_cur->path[S.state_cur->n_path++];
    path->fr = fr;
    path->to = to;
//fprintf(stderr,"PATH [%d]: %d -> %d\n", s->n, fr, to);
}
...
void ceu_sim_state_dump (tceu_sim_state* s)
{
    ...
    fprintf(S.file, "      path = { ");
    for (int i=0; i<s->n_path; i++) {
        fprintf(S.file, "{%d,%d},", s->path[i].fr, s->path[i].to);
    }
    fprintf(S.file, " },\n");
    fprintf(S.file, "    },\n");
}
