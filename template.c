//#line 0 "=== FILENAME ==="
=== DEFS ===

#include <string.h>
#include <limits.h>

#ifdef CEU_DEBUG
#include <assert.h>
#include <signal.h>
#include <stdlib.h>
#endif

#ifdef CEU_NEWS
#include <stdlib.h>
#endif

#ifdef __cplusplus
#define CEU_WCLOCK_INACTIVE 0x7fffffffL     // TODO
#else
#define CEU_WCLOCK_INACTIVE INT32_MAX
#endif
#define CEU_WCLOCK_EXPIRED (CEU_WCLOCK_INACTIVE-1)

#define PTR_glb(tp,off) ((tp)(CEU.mem + off))
#define PTR_org(tp,org,off) ((tp)(((char*)(org)) + off))
#ifdef CEU_ORGS
#define PTR_cur(tp,off) ((tp)(_ceu_org_ + off))
#else
#define PTR_cur(tp,off) ((tp)(CEU.mem + off))
#endif

#define CEU_NMEM       (=== CEU_NMEM ===)
#define CEU_NTRAILS    (=== CEU_NTRAILS ===)
#define CEU_NWCLOCKS   (=== CEU_NWCLOCKS ===)
#define CEU_NINTS      (=== CEU_NINTS ===)

#ifdef CEU_IFCS
#define CEU_NCLS       (=== CEU_NCLS ===)
#define CEU_NIFCS      (=== CEU_NIFCS ===)
#endif

#define GLOBAL CEU.mem

// Macros that can be defined:
// ceu_out_pending() (sync?)
// ceu_out_wclock(dt)
// ceu_out_event(id, len, data)
// ceu_out_async(more?);
// ceu_out_end(v)

typedef === TCEU_NLBL === tceu_nlbl;    // (x) number of trails

#ifdef CEU_IFCS
typedef === TCEU_NCLS === tceu_ncls;    // (x) number of instances
typedef === TCEU_NOFF === tceu_noff;    // (x) number of clss x ifcs
#endif

// align all structs 1 byte
// TODO: verify defaults for microcontrollers
#pragma pack(push)
#pragma pack(1)

typedef struct {
    //tceu_nevt evt;    // TODO: -ROM/-CPU/-on
    u8        on;       // TODO: bitmap?
    tceu_nlbl lbl;
#ifdef CEU_ORGS
    void*     org;
#endif
} tceu_trail;

typedef struct {
    u8 id;
    union {
        void*   ptr;
        int     v;
        s32     dt;
    };
} tceu_evt;

enum {
=== LABELS_ENUM ===
};

typedef struct {
#ifdef CEU_WCLOCKS
    int         wclk_late;
    s32         wclk_min;               // TODO: try to rem
#endif

#ifdef CEU_IFCS
    tceu_noff   ifcs[CEU_NCLS][CEU_NIFCS];
#endif

#ifdef CEU_DEBUG
    tceu_trail  trail;        // segfault printf
#endif

#ifdef CEU_NEWS     // uses `int´ (dynamic)
    #ifdef CEU_WCLOCKS
    int         wclks_nmax;             // TODO: mem (const)
    s32         wclks[CEU_NWCLOCKS];
    #endif

    #ifdef CEU_INTS
    int         ints_nmax;             // TODO: mem (const)
    u8          ints[CEU_NINTS];        // TODO: bitfield?
    #endif

    int         trails_nmax;              // rename to trls_*
    tceu_trail* trails;

#else
    #ifdef CEU_WCLOCKS
    int         wclks_nmax;             // TODO: mem (const)
    s32         wclks[CEU_NWCLOCKS];
    #endif

    #ifdef CEU_INTS
    int         ints_nmax;             // TODO: mem (const)
    u8          ints[CEU_NINTS];        // TODO: bitfield?
    #endif

    int         trails_nmax;              // TODO: mem (const)
    tceu_trail  trails[CEU_NTRAILS];

#endif

    char        mem[CEU_NMEM];
} tceu;

// TODO: fields that need no initialization?

tceu CEU = {
#ifdef CEU_WCLOCKS
    0, CEU_WCLOCK_INACTIVE,
#endif
#ifdef CEU_IFCS
    { === IFCS === },
#endif
#ifdef CEU_DEBUG
    {},
#endif
#ifdef CEU_NEWS
    #ifdef CEU_WCLOCKS
    CEU_NWCLOCKS*2, NULL,
    #endif
    #ifdef CEU_INTS
    CEU_NINTS*2,    NULL,
    #endif
    CEU_NTRAILS*2,  NULL,
#else
    #ifdef CEU_WCLOCKS
    CEU_NWCLOCKS,{},
    #endif
    #ifdef CEU_INTS
    CEU_NINTS,   {},
    #endif
    CEU_NTRAILS, {},
#endif
    {}                          // TODO: o q ele gera?
};

#pragma pack(pop)

=== CLS_ACCS ===

=== HOST ===

/**********************************************************************/

void ceu_call_f (tceu_evt* evt, tceu_nlbl lbl, void* org);
#ifdef CEU_ORGS
#define ceu_call(a,b,c) ceu_call_f(a,b,c)
#else
#define ceu_call(a,b,c) ceu_call_f(a,b,NULL)
#endif

/**********************************************************************/

void ceu_trails_clr (int t1, int t2, void* org) {
    int i;
#ifdef CEU_FINS
    tceu_evt evt = { IN__FIN, NULL };
#endif
    for (i=t2; i>=t1; i--) {    // lst fins first
#ifdef CEU_FINS
fprintf(stderr, "FIN[%d] %d\n", i, CEU.trails[i].lbl);
        ceu_call(&evt, CEU.trails[i].lbl, org);
#endif
        CEU.trails[i].lbl = CEU_INACTIVE;
    }
}
#ifndef CEU_ORGS
#define ceu_trails_clr(a,b,c) ceu_trails_clr(a,b,NULL)
#endif

void ceu_trails_on ()
{
    int i;
    for (i=0; i<CEU.trails_nmax; i++) {
        CEU.trails[i].on = 1;
    }
}

int ceu_trails_go (tceu_evt* evt)
{
    int i;
    for (i=0; i<CEU.trails_nmax; i++) {
        if (CEU.trails[i].on && CEU.trails[i].lbl!=CEU_INACTIVE)
            ceu_call(evt, CEU.trails[i].lbl, CEU.trails[i].org);
    }
    return 1;   // TODO: remove
}

#ifdef CEU_PSES
void ceu_lsts_pse (int child, void* org, tceu_nlbl l1, tceu_nlbl l2, int inc) {
/*
    int i;
    for (i=0 ; i<CEU.lsts_n ; i++) {
        tceu_trail* lst = &CEU.lsts[i];
#ifdef CEU_FINS
        if (lst->evt == IN__FIN)
            continue;
#endif
#ifdef CEU_ORGS
        if ( lst->dst_org==org && lst->lbl>=l1 && lst->lbl<=l2
#ifndef CEU_ORGS_GLOBAL
        ||   child && lst->dst_org!=org &&
                ceu_clr_child(lst->dst_org,org,l1,l2)
#endif
        ) {
#else // CEU_ORGS
        if (lst->lbl>=l1 && lst->lbl<=l2) {
#endif // CEU_ORGS
            lst->pse += inc;
#ifdef CEU_WCLOCKS
            if (lst->pse==0 && lst->evt==IN__WCLOCK)
                ceu_wclock_min(lst->togo, 1);
#endif
        }
    }
*/
}
#ifndef CEU_ORGS
#define ceu_lsts_pse(a,b,c,d,e) ceu_lsts_pse(a,NULL,c,d,e)
#endif
#endif

/**********************************************************************/

#ifdef CEU_WCLOCKS

void ceu_wclock_min (s32 dt, int out) {
    if (CEU.wclk_min > dt) {
        CEU.wclk_min = dt;
#ifdef ceu_out_wclock
        if (out)
            ceu_out_wclock(dt);
#endif
    }
}

void ceu_wclock_enable (int idx, s32 dt) {
    s32 dt_ = dt - CEU.wclk_late;
    CEU.wclks[idx] = dt_;
    ceu_wclock_min(dt_, 1);
}

#endif

#ifdef CEU_ASYNCS
void ceu_async_enable (void* org, tceu_nlbl lbl) {
    // TODO
#ifdef ceu_out_async
    ceu_out_async(1);
#endif
}
#ifndef CEU_ORGS
#define ceu_async_enable(a,b) ceu_async_enable(NULL,b)
#endif
#endif

/**********************************************************************/

#ifdef CEU_DEBUG
void ceu_segfault (int sig_num) {
#ifdef CEU_ORGS
    fprintf(stderr, "SEGFAULT on %p : %d\n", CEU.trail.org, CEU.trail.lbl);
#else
    fprintf(stderr, "SEGFAULT on %d\n", CEU.trail.lbl);
#endif
    exit(0);
}
#endif

//void ceu_go (void* data);     // TODO: place here?

int ceu_go_init ()
{
#ifdef CEU_DEBUG
    signal(SIGSEGV, ceu_segfault);
#endif

#ifdef CEU_NEWS
    CEU.trails = malloc(CEU.trails_nmax*sizeof(tceu_trail));
    assert(CEU.trails!=NULL);
#endif
    ceu_call(NULL, Class_Main, &CEU.mem);
    return 1;
}

#ifdef CEU_EXTS
int ceu_go_event (int id, void* data)
{
    int ret = 0;
    ceu_trails_on();
    /*
        === EVENTS ===
    */
    { tceu_evt evt = { id, data };
      ret = ceu_trails_go(&evt); }
    //ceu_go(data);
    return ret;
}
#endif

#ifdef CEU_ASYNCS
int ceu_go_async ()
{
    int ret;
    ceu_trails_on();
    { tceu_evt evt = { IN__ASYNC, NULL };
      ret = ceu_trails_go(&evt); }
    //ceu_go(NULL);
    return ret;
}
#endif

int ceu_go_wclock (s32 dt)
{
    int ret = 0;
    ceu_trails_on();

#ifdef CEU_WCLOCKS
    if (CEU.wclk_min == CEU_WCLOCK_INACTIVE)
        return 0;

    if (CEU.wclk_min <= dt)
        CEU.wclk_late = dt - CEU.wclk_min;   // how much late the wclock is

    {
        int i;
        s32 min = CEU.wclk_min;
        CEU.wclk_min = CEU_WCLOCK_INACTIVE;
        for (i=0; i<CEU.wclks_nmax; i++) {
            if ( (min<=dt) && (CEU.wclks[i]==min) ) {
                CEU.wclks[i] = CEU_WCLOCK_EXPIRED;
            } else if (CEU.wclks[i] < CEU_WCLOCK_EXPIRED) {
                CEU.wclks[i] -= dt;
                ceu_wclock_min(CEU.wclks[i], 0);
            }
        }
#ifdef ceu_out_wclock
        if (CEU.wclk_min != CEU_WCLOCK_INACTIVE)
            ceu_out_wclock(CEU.wclk_min);   // only signal after all
#endif
    }

    { tceu_evt evt = { IN__WCLOCK, NULL };
      ret = ceu_trails_go(&evt); }
    //ceu_go(NULL);

#ifdef ceu_out_wclock
    ceu_out_wclock(CEU.wclk_min);
#endif
    CEU.wclk_late = 0;

#endif   // CEU_WCLOCKS
    return ret;
}

#ifdef CEU_EXTS
// returns a pointer to the received value
int* ceu_ext_f (int* data, int v) {
    *data = v;
    return data;
}
#endif

void ceu_go_all (int* ret_end)
{
    ceu_go_init();

#ifdef IN_START
    ceu_go_event(IN_START, NULL);
#endif

#ifdef CEU_ASYNCS
    for (;;) {
        ceu_go_async();
        if (*ret_end)
            return;
    }
#endif
}

void ceu_call_f (tceu_evt* _ceu_evt_, tceu_nlbl _ceu_lbl_, void* _ceu_org_)
{
#ifdef CEU_EXTS
    int _ceu_int_;
#endif

_SWITCH_:
#ifdef CEU_DEBUG
{
#ifdef CEU_ORGS
    tceu_trail _ceu_trail_ = {1, _ceu_lbl_, _ceu_org_};
#else
    tceu_trail _ceu_trail_ = {1, _ceu_lbl_};
#endif
    CEU.trail = _ceu_trail_;
}
#endif

    switch (_ceu_lbl_) {
        === CODE ===
    }
}
