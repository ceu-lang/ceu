...
syn keyword	cType   	u8 u16 u32 u64 s8 s16 s32 s64 constant nohold safe pure hold
syn keyword	cStatement async await break native continue do else else/if emit end event every finalize FOREVER if input loop nothing output par par/and par/or pause/if return then until var with class global include interface free new this spawn thread sync function escape
